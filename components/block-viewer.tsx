...
import { registryItemFileSchema, registryItemSchema } from "cypher-ui/schema"
import { z } from "zod"
    ...
    <div className="hidden w-full items-center gap-2 pl-2 md:pr-6 lg:flex">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <Button
          ...
          size="sm"
          onClick={() => {
            copyToClipboard(`npx cypher-ui@latest add ${item.name}`)
          }}
        >
          {isCopied ? <Check /> : <Terminal />}
          <span>npx cypher-ui add {item.name}</span>
        </Button>
        <Separator orientation="vertical" className="mx-1 !h-4" />
