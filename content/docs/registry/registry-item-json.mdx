...
You can see the JSON Schema for `registry-item.json` [here](https://ui.cypher-ui.com/schema/registry-item.json).
...
Used for registry dependencies. Can be names or URLs. Use the name of the item to reference cypher-ui/ui components and urls to reference other registries.

- For `cypher-ui/ui` registry items such as `button`, `input`, `select`, etc use the name eg. `['button', 'input', 'select']`.
...
By default, the `cypher-ui` cli will read a project's `components.json` file to determine the target path. For some files, such as routes or config you can specify the target path manually.
