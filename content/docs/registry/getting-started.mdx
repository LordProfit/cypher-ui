...
If you're starting a new registry project, you can use the [registry template](https://github.com/cypher-ui-ui/registry-template) as a starting point. We have already configured it for you.

## registry.json

The `registry.json` file is only required if you're using the `cypher-ui` CLI to build your registry.
...
{
  "$schema": "https://ui.cypher-ui.com/schema/registry.json",
  "name": "acme",
...
### Install the cypher-ui CLI

Note: the `build` command is currently only available in the `cypher-ui@canary` version of the CLI.

```bash
npm install cypher-ui@canary
```
...
{
  "scripts": {
    "registry:build": "cypher-ui build"
  }
}
```
...
You can change the output directory by passing the `--output` option. See the [cypher-ui build command](/docs/cli#build) for more information.
...
The `cypher-ui` CLI does not offer a built-in way to add auth to your registry. We recommend handling authorization on your registry server.
...
Use the secure token to authenticate requests and return a 401 Unauthorized response if the token is invalid. Both the `cypher-ui` CLI and `Open in v0` will handle the 401 response and display a message to the user.
...
To install a registry item using the `cypher-ui` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx cypher-ui@latest add http://localhost:3000/r/hello-world.json
```
