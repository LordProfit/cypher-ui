...
```bash
npx cypher-ui@latest add calendar
```
...
After upgrading the `Calendar` component, you can install the new blocks by running the `cypher-ui@latest add` command.

```bash
npx cypher-ui@latest add calendar-02
```
