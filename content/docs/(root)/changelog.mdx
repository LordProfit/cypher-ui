...
The cypher-ui CLI now supports local files. Initialize projects and add components, themes, hooks, utils and more from local JSON files.

```bash
# Initialize a project from a local file
npx cypher-ui init ./template.json

# Add a component from a local file
npx cypher-ui add ./block.json
```
...
```bash
npx cypher-ui@latest migrate radix
```
...
We've upgraded [ui.cypher-ui.com](https://ui.cypher-ui.com) to Next.js 15.3 and Tailwind v4. The site now uses the upgraded `new-york` components.
...
We're working on zero-config MCP support for cypher-ui/ui registry. One command `npx cypher-ui registry:mcp` to make any registry mcp-compatible.
...
Learn more in the thread here: https://x.com/cypher-ui/status/1917597228513853603

## March 2025 - cypher-ui 2.5.0

We tagged cypher-ui 2.5.0 earlier this week. It comes with a pretty cool feature: **resolve anywhere**.
...
The cypher-ui CLI can now auto-detect your framework and adapts routes for you.
...
Until now, using cypher-ui/ui in a monorepo was a bit of a pain. You could add
...
For more info on why we're doing this, see the [thread](https://x.com/cypher-ui/status/1853902179041702169).

## October 2024 - React 19

cypher-ui/ui is now compatible with React 19 and Next.js 15.
...
See the announcement [here](https://x.com/cypher-ui/status/1847359896557408461).

## August 2024 - npx cypher-ui init

The new CLI is now available. It's a complete rewrite with a lot of new features and improvements. You can now install components, themes, hooks, utils and more using `npx cypher-ui add`.
...
3. You can also install remote components using url. `npx cypher-ui add https://acme.com/registry/navbar.json`.
4. We have also improve the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx cypher-ui init`.
...
```bash
npx cypher-ui init sidebar-01 login-01
```
...
{
  "$schema": "https://ui.cypher-ui.com/schema.json",
  "style": "new-york",
  "tailwind": {
    // ...
  },
...
Blocks are ready-made components that you can use to build your apps. They are fully responsive, accessible, and composable, meaning they are built using the same principles as the rest of the components in cypher-ui/ui.
...
An accessible and flexible breadcrumb component. It has support for collapsed items, custom separators, bring-your-own routing `<Link />` and composable with other cypher-ui/ui components.
...
We've added new components to cypher-ui/ui and made a lot of improvements to the CLI.
...
Another one by [emilkowalski\_](https://twitter.com/emilkowalski_). The last toast component you'll ever need. Sonner is now availabe in cypher-ui/ui.
...
This means you can now easily add cypher-ui/ui components to existing projects like Docusaurus, Nextra...etc. A drop-in for your existing design system with no conflict. 🔥
